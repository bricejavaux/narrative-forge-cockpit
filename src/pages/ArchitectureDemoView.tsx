...
/**
 * Demo-only architecture view. Loaded lazily so the dummyData import
 * is excluded from the Production Test bundle entry point.
 */
export default function ArchitectureDemoView() {
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      <div>
        <p className="editorial-eyebrow">Pilotage narratif · Demo</p>
        <h1 className="text-3xl editorial-heading text-foreground mt-1">Architecture Tome — Demo</h1>
        <p className="text-xs text-amber mt-1">Mode Demo — données fictives uniquement.</p>
      </div>
      ...
      {!['Chapitres', 'Arcs globaux', 'Beats', 'Révélations', 'Payoffs', 'Conséquences'].includes(activeTab) && (
        <div className="cockpit-card p-8 text-center space-y-2">
          <p className="text-foreground text-sm">Vue "{activeTab}" — démonstration uniquement</p>
        </div>
      )}

      <NoteComposer target={`architecture demo · ${activeTab}`} />
    </div>
  );
}
