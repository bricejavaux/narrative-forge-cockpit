...
export default function WarningBanner({ warnings }: WarningBannerProps) {
  return (
    <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4 mb-6 animate-slide-in">
      <div className="flex items-center gap-2 mb-3">
        <AlertTriangle size={16} className="text-destructive" />
        <span className="font-display font-semibold text-sm text-destructive">
          Alertes système — Prototype non connecté
        </span>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
        {warnings.map((w, i) => (
          <div key={i} className="flex items-start gap-2 text-xs">
            ...
            <span className={w.severity === 'critical' ? 'text-destructive' : 'text-muted-foreground'}>
              {w.text}
            </span>
          </div>
        ))}
